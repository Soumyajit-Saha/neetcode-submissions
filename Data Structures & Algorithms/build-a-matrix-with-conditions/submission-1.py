class Solution:
    def buildMatrix(self, k: int, rowConditions: List[List[int]], colConditions: List[List[int]]) -> List[List[int]]:
        # get the ordering of row and col values using Kahn's algo
        # Then for each value [1, k] get the index i in row order and j in col order
        # use [i, j] as a position to put values 1 to k in res
        def topoSort(edges):
